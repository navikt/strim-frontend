...
function getEventDate(e: EventDto): Date | null {
    try {
        return e.startTime ? new Date(e.startTime) : null;
    } catch {
        return null;
    }
}
...
function getEventTags(e: EventDto): string[] {
    if (!Array.isArray(e.categories)) return [];
    return e.categories
        .map((c: any) => (typeof c === "string" ? c : c?.name))
        .filter(Boolean);
}

export default function EierMoterPage() {
    ...
    useEffect(() => {
        async function fetchEvents() {
            try {
                const res = await fetch("/api/events/owned");
                if (!res.ok) throw new Error("Failed to fetch owned events");
        ...
        <div className="container relative mx-auto pt-12 pb-12 space-y-12">
            ...
            <section>
                <div className="flex items-center justify-between mb-6 gap-4">
                    <h1 className="text-3xl font-bold">Møter du arrangerer</h1>
