...
async function forwardPatch(request: Request, id: string) {
    ...
    const apiUrl = backendUrl(`/events/${id}`);
...
async function forwardDelete(request: Request, id: string) {
    const reqId = (globalThis.crypto?.randomUUID?.() ?? `${Date.now()}-${Math.random()}`).toString();
    const logPrefix = `[api/events/${id} DELETE]`;
    const apiUrl = backendUrl(`/events/${id}`);
...
export async function DELETE(
    ...
    const { id } = await params;
    return forwardDelete(request, id);
}
