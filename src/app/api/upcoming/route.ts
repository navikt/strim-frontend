...
export async function GET(request: Request) {
    const apiUrl =
        process.env.NODE_ENV === 'production'
            ? 'http://strim-backend/events/upcoming'
            : 'http://localhost:8080/events/upcoming';

    console.log(`Fetching UPCOMING events from API URL: ${apiUrl}`);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error('Backend responded with error (upcoming)', {
        ...
        console.log('Backend UPCOMING events response:', JSON.stringify(data, null, 2));

        return NextResponse.json(data);
    } catch (error) {
        console.error('Error calling backend /events/upcoming:', error);
        return NextResponse.json(
